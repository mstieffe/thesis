...
The application of DBM to the high-temperature data set at $568$ K has shown that the method is able to reproduce local- and large-scale structural features as well as energetic properties of the reference system remarkably well. As such, DBM is capable of generating Boltzmann distributed configurations for the state point it was trained on. On the other hand, the baseline method based on energy minimization over stabilizes the system and therefore does not account for the diversity of microstates at a specific canonical state point. Consequently, further MD simulations controlled by a thermostat are required to recover the right state point.
...
Evaluating large-scale structural features beyond pair-statistics is challenging, since the high dimensionality of the system does not allow to directly visualize the configurational space. For this reason, dimensionality reduction is used to further examine the model's accuracy at higher order. However, linear dimensionality reduction techniques, such as principle component analysis, is insufficient to capture the global structure of data obtained from MD trajectories.\cite{} 

Therefore, a dimensionality reduction algorithm called \textit{Sketch-Map} (SM) is applied to probe the model's accuracy at higher order. SM builds a two-dimensional map representing proximity relationships between sPS monomers and their environments. 

\subsubsection{MD Simulation}

\subsection{Discussion}

However, DBM retains its performance displayed in Sec. \ref{SEC:DBM_general_performance} and reproduces the reference distributions at different state points with remarkable accuracy.
