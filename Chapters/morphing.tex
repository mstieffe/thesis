...
An additional test of the morphing procedure is demonstrated for the Martini model. As described in Sec. \ref{}, the Martini forcefield is a generic coarse-grained forcefield for a wide range of soft matter systems with an emphasis an biomolecules. The parametrization of the forcefield is based on the top-down approach for nonbonded interactions and on the bottom-up approach for bonded interactions. The Martini model is widely used due to its robust transferability across soft matter sytems. However, a major drawback of the Martini model is a less accurate reproduction of structural features.

\subsection{Set-up and Reference Data}

The Martini model is deployed for a MD simulation of tetracosane (TCS). TCS is an alkane hydrocarbon with the structural formula $H(CH_2)_{24}H$. Applying the Martini mapping rules, the coarse-grained TCS molecule concists of six beads of the apolar type C. A MD simulation of a TCS liquid based on the Martini forcefield is carried out in the NPT ensemble at $T = 400$K using the molecular dynamics package GROMACS 5.0.\cite{hess2008gromacs} Temperature and pressure of the system are controlled using the velocity rescaling thermostat and the Parrinello-Rahman barostat. An integration timestep of $10$ fs is used and samples are recorded every $0.5$ns. The simulation box contains 168 molecules.
...
Morphing of the local statistics for snapshots obtained with the Martini model is performed for $s = 2,3$. The training set for DBM consists of 16 snapshots obtained with the coarse-grained AA simulation. For validation, 104 samples of the Martini model are morphed deploying both schemes outlined in \ref{} and 104 samples of the coarse-grained AA simulation are used as a reference.

\subsubsection{Structural Distributions}

Fig. \ref{} displays structural distributions obtained for the coarse-grained AA simulation, Martini simulation on morphed structures. The distributions of the bond length in Fig. \ref{} (a) and (b) differ significantly for coarse-grained AA and Martini structures. While the Martini models yields a broad and gaussian shaped bond length distribution that extends from $\approx 0.30$nm to $\approx 0.60$nm, the distribution obtained for AA simulation is more structured and compressed. It extends from $\approx 0.38$nm to $\approx 0.54$nm and has two peaks at $\approx 0.47$nm and $\approx 0.52$nm. While DBM is able to compress the bond length distribution to the range of the coarse-grained AA target system, it is not capable of recovering the specfic structure of the distribution. Specifically, it fails to reproduce the second peak at $\approx 0.52$nm. The best match with the target distribution is visually obtained for morphing scheme A and $s=2$.

The angle distribution displayed in Fig. \ref{} (c) and (d) follow a similar trend. While the distributions obtained for the Martini and the coarse-grained AA simulation cover a similar range, the later is more complex. The Martini distributions displays a single peak at $\approx 135 ^{\circ}$ and suppresses large angles $> 170^{\circ}$, whereas the mapped AA simulation displays two peaks at $\approx 140 ^{\circ}$ and $\approx 170 ^{\circ}$. None of the morphing models is able to correct the angle distribution sufficiently. The best result is obtained visually for scheme A and $s=3$, which slightly shifts the distributions towards larger angles.

The pair correlation function $g(r)$ depicted in Fig. \ref{} (e) and (f) is sharply peaked for the Martini model. The coarse-grained AA configurations yield a less peaked distribution and slightly shorter minimum distance between nonbonded beads. Most morphing schemes are able to smooth the pair correlation function except for scheme B and $s=2$. The best match is visually obtained for scheme B and $s=3$.
...
Similarly to the analysis in Sec. \ref{}, free energy landscapes in the two-dimensional sketch-map space are computed. To this end, landmarks are obtained for 672 molecules sampled from both, the Martini as well as the coarse-grained AA simulation. For each free energy distribution 16800 molecules are projected onto the sketch-map space guided by the landmarks. Relative populations are computed over 50 bins along each dimension.

The obtained free energy landscapes are depicted in Fig. \ref{}. The configurations obtained with the coarse-grained AA model (panel (a)) yield a densely distributed free energy landscape that covers the upper third of the second sketch-map axis $a_2$. Further analysis of selected molecular conformations reveal that the area associated with the highest occupation at the center of the displayed range of $a_1$ correspond to u-shaped molecules (example (2)). Lower values of $a_1$ can be associated with rather streched molecular conformations (example (1)), while higher values of $a_1$ can be associated with rather rigid conformations containing a kink (example (3)). The occupied region corresponding to the lowest value of $a_2$ can be associated with collapsed molecular conformations (example (4)).

The free energy landscape obtained for the Martini model displayed in panel (b) covers a broader range compared to the coarse-grained AA model. While most of the probability mass is still centered at higher values for $a_2$, almost the full range of $a_2$ is covered. Note that the occupied area of the coarse-grained AA model is completly covered by the Martini model indicating a richer conformational space. Example structures (5) and (7) drawn from areas not occupied by the AA model correspond to condensed conformations or zig-zag structures, respectively.

Comparing panels (a) and (b) reveals the challenges for the morphing task. The rich conformational space obtained with the Martini model has to be compressed and projected onto those regions of conformational space covered by the AA model ideally reproducing the correct relative populations. Analysing panels (c)-(f) indicate that this task is only partly successfull. All morphing models are able to shift the probably mass towards higher values of $a_2$, i.e. pushing it closer to the target distribution. While this procedure is successfull for some conformations (example (6)), it fails for others (example (8)). 
...
Identifying the best performing morphing model via visual inspection of the structural distributions in Fig. \ref{} and the free energy landscapes in Fig. \ref{} is challenging. Therefore, the Jenson-Shannon divergence between the free energy distribution for the target coarse-grained AA simulation and the morphed structures is displayed in Table \ref{}. According to the Jenson-Shannon divergence scheme B and $s=3$ yields the best match.
...
\label{TAB:sPS_chem_trans}
\end{tabular}
\end{table}expose german

\subsubsection{Backmapping}

Following the primary intention of the project, morphed structures are backmapped to the AA resolution. To this end, DBM is trained on 16 snapshots of the AA model to reintroduce missing degrees of freedom. The model is trained using regularization $\mathcal{C}^{(1)}_{\text{pot}}$ based on the Gromos forcefield. After training, backmapping is performed for coarse-grained AA configurations, Martini configurations, and morphed Martini configurations. Morphed structures are obtained deploying the best performing morphing model, i.e. schema B and $s=3$. 

Fig. \ref{} displays structural distributions of the carbon atoms for the reference AA and backmapped configurations. The baseline accuracy of DBM is probed by its ability to backmap coarse-grained AA configurations. The bond length distribution depicted in panel (a) is reproduced with high accuracy, whereas the angle distribution shown in panel (b) is slightly too broad. However, the small range of angles of the reference AA simulation has to the be emphazied. The pair correlation function $g(r)$ displayed in panel (c) is reproduced with high accuracy.

The quality of structures backmapped from the Martini model differs significantly from the baseline quality. Unfortunately, morphing of the Martini model does only yield a minor improvement. The peaks for the bond length and angle distribution slightly increases upon morphing. Most noticeable, the $g(r)$ indicates that too small distances of nonbonded carbon pairs are removed.
