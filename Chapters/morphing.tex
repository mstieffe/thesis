...
Top-down coarse-grained models aim at reproducing certain experimentally observed phenomena or to study the implications of applying general rules. As such, top-down coarse-grained models are typically not chemically specific. However, in many cases bottom-up aspects are incorporated in the parameterization of the coarse-grained model as well. Examples of such hybrid models include the KG polymer model with a tuned bending potential, which allows to establish a relation to real polymers at the Kuhn scale, and the Martini forcefield, where a structure-based method is used to tune bonded interactions. While those models can be related to a specific chemistry, they typically lack the structural fidelity of solely structure-based coarse-grained models. 
%This limitation becomes especially apparent for a direct comparison of representations obtained with such hybrid top-down models and more faithful representations obtained with higher resolution model mapped to the coarse-grained resolution.

In this chapter, a data-driven method to improve the quality of molecular structures obtained with hybrid top-down models is investigated that does not rely on forcefield parameterization. To this end, a ML model is deployed to learn the residuals between the obtained coarse-grained distribution and a target distribution. An illustration of the method can be found in Fig. \ref{FIG:morph_intro}. In particular, it is assumed that the model distribution already captures the correct large-scale characteristics of a target system. As such, the method aims at morphing local statistics in order to resemble the target distribution more closely. To this end, a local scale is introduced that defines the extent to which features are allowed to change. Specifically, molecular structures are further coarse-grained up to this scale. Afterwards, DBM is applied to reintroduce local features learned from examples of the target system. 
