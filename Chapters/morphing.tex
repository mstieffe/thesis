...
The morphing approach is applied to a sPS polymer melt sampled with the KG model with tuned bending potential and a condensed-phase system of the alkane tetracosane sampled with the Martini force field. For each system, two molecular models are deployed, (1) a top-down model, which is subject to the morphing and (2) a higher resolution model, which yields a target distribution to train the ML model.
...
The Martini model is deployed for a MD simulation of TCS, which is an alkane hydrocarbon with the structural formula $H(CH_2)_{24}H$. Applying the Martini mapping rules, the CG TCS molecule consists of six beads of the apolar type C. A MD simulation of a TCS liquid based on the Martini force field is carried out in the $NPT$ ensemble at $T = 400$ K using the molecular dynamics package GROMACS 5.0 \cite{hess2008gromacs}. Temperature and pressure of the system are controlled using the velocity rescaling thermostat and the Parrinello-Rahman barostat. An integration time step of $10$ fs is used and samples are recorded every $0.5$ ns. The simulation box contains 168 molecules.

To obtain a target distribution, an all-atom (AA) simulation of TCS is performed. To this end, the GROMOS force field is used and topologies are generated by {\sc Automated Topology builder} \cite{malde2011automated}. The MD settings are equivalent to the settings for the Martini simulation, except for a reduced integration time step of $1$ fs. The AA simulation is mapped onto the resolution of the Martini model by mapping every four carbon atoms and associated hydrogens to their center of mass.
...
Following the primary intention of the project, morphed structures are backmapped to the AA resolution. To this end, DBM is trained on 16 snapshots of the AA model to reintroduce missing degrees of freedom. The model is trained using regularization $\mathcal{C}^{(1)}_{\text{pot}}$ based on the Gromos force field. After training, backmapping is performed for CG AA configurations, Martini configurations, and morphed Martini configurations. Morphed structures are obtained deploying the best performing morphing model, i.e. scheme B and $s=3$. 
