...
ML is an umbrella term for many different algorithms, such as Kernel methods, decision trees or Artificial Neural Networks (ANNs). Throughout this work, ANNs are the main ML method used. Therefore, the main body of this chapter focuses on an introduction of ANNs. The rest of this chapter is structured as follows: Firstly, some general remarks important to distinguish ML models will be made, then ANNs are explained in detail and finally the clustering algorithm Sketch-Map is introduced.

\section{General remarks}

ML algorithms can be divided into many different categories. First of all, they can be split into supervised and unsupervised learning approaches. The supervised learning approach uses labeled data, where each input is linked to a desired output. Typical examples of supervised learning are regression or classification tasks. The unsupervised approach deals with unlabeled data, where only the inputs are given, and aims to find structure in the data, like clustering the data points.

Another common distinction is made between generative and discriminative ML models. While the former learns a probability distribution over all variables in the data, the latter learns decision boundaries between different classes of the data for regression or classification. 

Formally, consider a set of labelled data $\{(x,y)\}$ drawn from a joint propability distribution $p(x,y)$, where $x$ is an observation and $y$ the corresponding class label. A discriminative model learns the conditional probability of the class labels given the obersavation $p(y|x)$ or directly maps oberservations $x$ to a class label $y$. Therefore, a discriminative model does not need to learn about the depencies of all the variables in the system but focuses only on those variables important to label the observations.

Generative approaches on the other hand model the joint probability $p(x,y)$. They are framed generative as the probability $p(x,y)$ can be used to generate new instances of the underlying system. However, it is straighforward to compute marginalized distributions $p(x) = \sum_y p(x,y)$ or conditional distributions $p(y|x) = \frac{p(x,y)}{p(x)}$. Therefore, generative models can be used for classification as well. Generative models also include implicit models that do not model $p(x,y)$ explicitly but introduce a generator to create new samples that infer the underlying probability distribution of the training data. 

Due to the higher complexity of the task, a generative model typically needs more parameters than a discriminative model. Regarding logistic regression (discriminative approach) and Gaussian Naive Bayes (generative approach), experiments have shown that discriminative approaches lead to lower asymptotic errors compared to generative models. However, Gaussian Naive Bayes outperforms logistic regression in a low data regime.
