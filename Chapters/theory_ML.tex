...
\subsection{Latent Variables and the Manifold Hypothesis}
...
Generative models can be further distinguished between those that define the models probability distribution $p_{M}(x;\Theta))$ explicitly through some functional form and those that define it implicitly through a sampler that allows to draw samples from it. The former approach has the benefit that maximizing the likelihood $\mathcal{L}$ is straigh-forward, as the probability distribution can be accessed directly. However, explicit models require to assume some functional form for the probability $p_{M}(x;\Theta))$, which often becomes a bottleneck for the expressive power of the model. Therefore, designing an explicit model is often a trade-off between capturing the complexity of the true data distribution and still being tractable.

The various variants of explicit DGMs fall into two categories: Models that carefully construct a tractable functional form for the likelihood $\mathcal{L}$ and models that use a tractable approximation. Two major examples for tractable explicit models are autoregressive models and normalizing flow models. The most famous example for an explicit model that requires approximation is the variational auto encoder.
...
where $p_{\mathrm{Z}(z)}$ is prior distribution for the latent variables. The posterior distribution $p_{\Theta}(z|x)$ is typically intractable and VAE approach continues to approximate it within a family of parameterized probability distributions that are tractable. This is achieved using a NN $e_{\Psi}(z|x)$ (encoder) that predicts the parameters for the approximate posterior,

\begin{equation}
  e_{\Psi}(z|x) \approx p_{\Theta}(z|x) ,
\end{equation}
  
where $\Psi$ are the parameters for the NN. 

To train both, the decoder $g_{\Theta}$ and the encoder $e_{\Psi}$, a tractable lower bound for the evidence (ELBO) can be derived

\begin{align}
  log(p_{\Theta}(x)) &\geq \mathbb{E}_{e_{\Psi}(z|x)} \Big[ log \big( \frac{p_{\Theta}(x,z)}{e_{\Psi}(z|x)} \big) \Big] \\
    &=  \mathbb{E}_{e_{\Psi}(z|x)} \Big[ log \big( p_{\Theta}(x|z)\big) \Big] - \text{KL}(e_{\Psi}(z|x)||p_{\mathrm{Z}(z)})  
\end{align}
  
and maximized. The first term reduces the approximation error in ambient space (reconstruction error) that arises due to the restriction for the dimensionality of the latent space and the approximation error for the posterior. The second term acts as a regularizer that biases the the approximate posterior towars the assumend prior distribution $p_{\mathrm{Z}(z)}$. Note that both terms might be in a conflict with each other. 

The main drawback of VAEs is that the gap between the ELBO used for optimization and the actual likelihood might be very big resulting in a model that is very different from the true distribution. Empically, vanilla VAEs have a tendency to ignore some of the latent variables and produce blurred samples in ambient space.
  
\subsubsection{Markov Chain approximation}
  
Some generative models generate samples using a markov chain technique: A sample $x$ is repeatly updated according to according to some transition operator $x' \sim q(x'|x)$. 

An example for such a model is the Boltzmann machine (BM): A BM concists of binary units connected with each other. The weights $\Theta$ of the network are the parameters of an energy function $E(\Theta)$ which is used to define the probability distribution over binary patterns of the units. During training, the weights are updated such the likelihood for the given training data is maximized. To sample a new state the units are repeatly updated stochastically until an equilibrium state is reached. 
