...
Generative models are designed to learn an esitimate for a distribution $\mathcal{X}$ defined over $\mathbb{R}^D$, where the dimension $D$ is typically large. In general, the distribution $\mathcal{X}$ is complicated and intractable. In practice, models are trained using a set of indipendently and identically distributed training samples $\{x\}$, where the samples $x \sim \mathcal{X}$. The probability for a sample $x$ is denoted with $p_{\mathcal{X}}(x)$ and the probability defined by the model is $p_{\Theta}(x)$, where $\Theta$ refers to the parameters of the model. The overall goal of a generative model is to optimize the parameters such the model probability $p_{\Theta}(x)$ converges to the true probability $p_{\mathcal{X}}(x)$. 

\subsection{Discriminative vs generative models}

The definition for generative models is often opposed to discriminative models. While the former aims to learn about the dependencies of all the variables in a system, the latter learns to introduce decision boundaries for different classes. Generative models are typically linked to unsupervised training and discriminative models to supervised trainig, but a clear dichotomy is not always possible.

Formally, consider a set of labelled data $\{(x,y)\}$ following the joint probability distribution $p(x,y)$, where $x$ is an observation and $y$ the corresponding class label. A discriminative model learns the conditional probability $p(y|x)$ of the class labels $y$ given the obsersvation $x$. This can be achieved either explicitly or implicitly by directly mapping the oberservations $x$ to a class label $y$. Therefore, a discriminative model does not necessarily have to learn about the depencies of all the variables in the system but only on those variables important to label the observations.

Generative approaches on the other hand aim to model the joint probability $p(x,y)$ (or simply $p(x)$ if no labels are given). They are framed generative as the probability $p(x,y)$ can be used to generate new instances of the underlying system. However, it is straighforward to compute marginalized probabilities $p(x) = \sum_y p(x,y)$ or conditional probabilities $p(y|x) = \frac{p(x,y)}{p(x)}$. Therefore, generative models can be used for classification as well. 

Obviously, the underlying task for generative models is more complex compared to the discriminative task as the generative model has to be more informative. Therefore, generative models typically require more parameters than discriminative models. Furthermore, experiments comparing logistic regression (discriminative model) with Gaussian Naive Bayes (generative model) for a binary classification task have shown that the former outperforms the latter in terms of asymptotic error. However, in a low data regime generative models can give superior performance. 

While both approaches can be used for classification, only the generative approach can be used to sample new instances of the data which is a mandatory requirement for the overall goal of this thesis. Therefore, the rest of this chapter will focus on the generative approach. More specifically, the following discussion is restricted to Deep Generative Models (DGMs) for simplicity.

\subsection{Maximum likelihood approach}

The major route to train a DGM is to maximize the data likelihood. The underlying idea of this approach is to find a model that best explains the oberserved data . A model with parameters $\Theta$ provides an estimate for the probability $p_{M}(x;\Theta)$ for a given oberservation $x$. The likelihood $L$ for the data under the parametric model can be written as

\begin{equation}
  \mathcal{L} = \prod_i^N p_{M}(x;\Theta) ,
\end{equation}
...
The cross-entropy takes a minimum when the two distributions match, that is $p_{M}(x;\Theta) = p_{D}(x)$. Therefore, an ideal model trained with the maximum likehood approach yields a model that perfectly reproduces the true data distribution.

\subsection{Latent variables}

Latent variable models (LVM) assume that real-world data is generally structured and therefore can be described through a lower-dimensional representation. This assumption is known as the manifold hypothesis, which states that examples $x \in \mathbb{R}^D$ lie on a low-dimensional manifold $\mathcal{M} \subset \mathbb{R}^D$ embedded in the high-dimensional space $\mathbb{R}^D$. Therefore, real-world data is assumed to have an intrinsic lower dimensionality $d$ and our oberservations $x$ can be explained through some hidden variables $z \in \mathbb{R}^d$. The high-dimensional space $\mathbb{R}^D$ is called ambient space and the low-dimensional space $\mathbb{R}^d$ is referred to as latent space.

The idea of LVM is to decompose the generative process for new samples that are consistent with the training set $\{x | x \in \mathbb{R}^D\}$ into two steps: (1) Draw a sample $z \in \mathbb{R}^d$ from the prior distribution $p(z)$. (2) Use the conditional probability $p(x|z)$ to generate a new sample. This can be summarized as 

\begin{equation}
  x \sim p(x|z) \; , \; \; z \sim p(z)  .
\end{equation}

A key problem of LVM is to infer the posterior distribution of the latent variables $p(z|x)$. Using Bays's rule we can write
...
The autoregressive approach uses the chain rule for probabilities to rewrite the joint probability $p(x)$ of an n-dimensional vector $\mathbf{x}$ into a product of conditional probabilities that are easy to access

\begin{equation}
  p_{M}(\mathbf{x}) = \prod_i^n p(x_i | x_1, x_2, .., x_{i-1}) .
\end{equation}

Splitting up the problem into conditional probailities allows for a tractable explicit model. The drawback of such autoregressive models is that they can only generate one entry at a time prohibiting parallel computation. However, this approach is well suited for data that is sequential in nature, such as human speech.

\subsubsection{Normalizing Flow}

Normalizing Flow (NF) models are LVM models that make use of the change of variables formula for invertable transformations $g$ that map from one space $\mathrm{X}$ (the data space) to another $\mathrm{Z}$ (the latent space). Given a function $g: \mathrm{R}^n \rightarrow \mathrm{R}^n$ that is invertible and both $g$ and $g^{-1}$ are continuously differentiable, as well as orientation-preserving ($ \nabla g > 0 $), then the change of variables formula can be used to express the likelihood of a data point $x \sim \mathrm{X}$ in terms of another, potentially simpler, density function in the latent space

\begin{equation}
  p_{\mathrm{X}} (\mathbf{x}) = p_{\mathrm{Z}} (g^{-1}(\mathbf{x})) \Big| \text{det} \left( \frac{\partial g^{-1}(\mathbf{x})}{\partial \mathbf{x}} \right) \Big| .
\end{equation}

NF models have the advantage that they allow for direct optimization and evaluation of the likelihood. The major drawback of NF models are the required restrictions on the transformation $g$ that are difficult to fulfill in practice limiting their applicability. The most important restriction to be named is the equality of the dimensions of the latent and the data space.

\subsubsection{Variational Autoencoder}

A Variational Autoencoder (VAE) is a LVM consisting of two parts: An encoder $e_{\Psi}(z|x)$ compresses a given input $x$ into a constraint distribution in the latent space $\mathrm{Z}$ and a decoder $p_{\Theta}(x|z)$ reconstructs a distribution in the ambient space $\mathrm{X}$. Typically, $e_{\Psi}(z|x)$ and $p_{\Theta}(x|z)$ are NNs with parameters $\Psi$ and $\Theta$ respectively that predict parameters for a family of parametrized distributions. Importantly, unlike NF models the dimensions of latent and ambient space do not have to match. Typically the dimension of the latent space is much smaller than the dimension of the data space and a simple Gaussian distribution is used as a prior distribution $p_{\mathrm{Z}}(z)$. As a consequence the mapping between the spaces is not invertable anymore and thus the likelihood can not be computed directly.
...
Using Bayes's rule the likelihood for a data point $x$ can be written as

\begin{equation}
  p_{\Theta}(x) = \frac{p_{\Theta}(x|z)p_{\mathrm{Z}}(z)}{p_{\Theta}(z|x)} .
\end{equation}

The posterior distribution $p_{\Theta}(z|x)$ is typically intractable and the VAE approach aims to approximate it with the encoder $e_{\Psi}(z|x)$,
...
\begin{align}
  log(p_{\Theta}(x)) &\geq \mathbb{E}_{e_{\Psi}(z|x)} \Big[ log \big( \frac{p_{\Theta}(x,z)}{e_{\Psi}(z|x)} \big) \Big] \\
    &= \underbrace{ \mathbb{E}_{e_{\Psi}(z|x)} \Big[ log \big( p_{\Theta}(x|z)\big) \Big] } _\text{reconstruction} -  \underbrace{  \text{KL}(e_{\Psi}(z|x)||p_{\mathrm{Z}(z)})  }_\text{regularization}
\end{align}
  
The negative ELBO is then minimized. The first term reduces the approximation error in ambient space (reconstruction error) that arises due to the restriction for the dimensionality of the latent space and the approximation error for the posterior. The second term acts as a regularizer that biases the the approximate posterior towars the prior distribution $p_{\mathrm{Z}}(z)$. Note that both terms might be in a conflict with each other. 
...
An example for such a model is the Boltzmann machine (BM): A BM concists of binary units connected with each other. The weights $\Theta$ of the network are the parameters of an energy function $E(\Theta)$ which is used to define the probability distribution over binary patterns of the units. During training, the weights are updated such that the likelihood for the given training data is maximized. To sample a new state the units are repeatly updated stochastically until an equilibrium state is reached. 
...
Generative adversarial networks (GANs) were introduced by Ian Goodfellow et. al in 2014. They have become one of the most successful implicit generative models known in ML community. In its core, a GAN concists of two competing models trained in a game: A generator $G$ produces samples $x'$ aiming to reproduce the distribution $p(x)$ of a training set $\{x\}$. A second model, the discriminator $C$, has to distinguish between synthetic samples from the generator and real samples from the training set. Thereby, the discriminator $C$ acts as a distance measure for the real distribution $p(x)$ and the distribution of synthetic samples $p(x')$. While the discriminator $C$ is trained as a classifier in a supervised manner, the generator $G$ is trained fool the discriminator $C$. Therefore, the generator $G$ is indirectly pushed towards minimizing the difference between $p(x)$ and $p(x')$.
The whole process of training a GAN is concidered a likelihood-free method as neither the likelihood of sample itself nor a lower bound of it is used. 

A GAN is a LVM, but unlike VAEs or NF models they are not aiming to infer the distribution of latent variables underlie the samples, but learn a transformation from a given prior distribution $p(z)$ to $p(x)$.
Both, the generator and the discriminator are typically implemented as DNNs. The generator $G$ is defined as a function over latent samples $z \in \mathrm{R}^{d}$ from a prior distribution $p(z)$,

\begin{equation}
  G(\Theta): \mathrm{R}^{d} \rightarrow \mathrm{R}^{D}
\end{equation}

where $\Theta$ are the parameters of the generator, $d$ is the dimension of the latent variable $z$ and $D$ is the dimension of the ambient space for observations $x$. The prior distribution $p(z)$ is typically defined as a high-dimensional Gaussian distribution or uniform distribution over a hypercube. The latent samples $z$ are used a source of randomness for the model and $G(z;\Theta)$ transforms the given noise $z$ into synthetic samples $x'$.

The main goal of GANs is to converge the distribution of synthetic samples $p(x')$ implicitly defined by the generator $G(z;\Theta)$ towards the real distribution of the training examples $p(x)$. 

%Naive approaches would continue to define an objective for the generator that minimizes the difference between the two probability distributions $p(x)$ and $p(x')$, such as the cross entropy, or apply a direct maximum likelihood training, where the model’s parameters are tuned such that the likelihood of observing the data given the model is optimized. Unfortunately, in many cases we do not have direct access to the explicit probability densities $p(x)$ and $p(x')$ because the high dimensionality of the data makes computing the normalization factor (the partition function) unfeasible.
