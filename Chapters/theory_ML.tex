...
as well as the \textit{relative entropy}
...
The whole training process of a GAN is considered a likelihood-free method as neither the likelihood of the model $p_{\mathcal{\Theta}}(\mathbf{x})$ itself nor a lower bound of it is used explicitly. 
...
Using the loss defined in \ref{gan_loss_binary}, it can be shown that the optimal discriminator $c_{\Psi^{*}, \Theta}$ for a fixed generator $g_{\Theta}$ is given by
