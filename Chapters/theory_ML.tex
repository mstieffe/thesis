...
Recently, ML is gaining significant attention in many fields of modern science, especially computational chemistry and particle physics \cite{noe2020machine, vamathevan2019applications, radovic2018machine}. Beside the massive increase in computational power, the growing interest for ML algorithms in those research areas is fueled by the availability of large data sets \cite{hachmann2011harvard, jain2013commentary, calderon2015aflow}. The massive amount of raw data collected in experiments or computer simulations demands for efficient algorithms to process and analyze it. Self-learning algorithms that improve their performance with increased data set size are therefore very appealing. This is especially true when the data is high-dimensional, as ML algorithms can spot complex patterns or reduce the dimensionality for further processing. 

Data analysis is also a hallmark of classical statistics. Indeed, ML and classical statistics are related and many techniques and concepts used in ML have their origin in physics, such as variational methods, simulated annealing or Monte-Carlo methods \cite{mehta2019high}. A famous example is the Boltzmann Machine, which has a direct analogy to a spin glass model known from statistical mechanics \cite{ackley1985learning}. However, despite their similarities, ML and classical statistics differ in their general philosophy: Central to most ML approaches is the transferability of the model, i.e. ML algorithms are typically designed to make predictions for new observations that are not part of the current data set \cite{mehta2019high}. On the other hand, methods of classical statistics are more concerned with estimation problems, i.e. to discover dependencies between variables of the current data set.

In this thesis, a ML model is used to make predictions for a fine-grained state of a molecular system based on a coarse-grained state. This chapter introduces important ML concepts required to build such a ML-based reverse-mapping model. While ML is an umbrella term for a wide spectrum of algorithms, such as Kernel methods, decision trees or deep neural networks (DNNs), it is out of scope of this chapter to cover all branches in this field. The interested reader is therefore referred to one of the many excellent books available \cite{sarker2021machine, bonaccorso2017machine, ayodele2010types}. Instead, this chapter focuses on generative modeling using DNNs, which is the main method used in this thesis. The rest of this chapter is organized as follows: Firstly, some basics of ML are introduced. Secondly, DNNs are explained in detail and finally, concepts of generative modeling are outlined. 

%difference of in-distribution and out-of-distribution error represents the difference between fitting and prediction.
...
\end{equation}
%
 with parameters $\Theta$ that maps the input $\mathbf{x}$ to some output $\mathbf{\hat{y}} \in \mathrm{R}^S$ with dimension $S$. The last ingredient required to train the model is a \textit{cost function} $\mathcal{C}(f_{\Theta}(\mathbf{x}))$
 %
\begin{equation}
...
%In this work, $\mathbf{x}$ typically refers to a molecular configuration $\mathbf{r} \in \mathrm{R}^{3N}$ of $N$ particles.
\subsection{Interpretation of Probability: Bayesian vs Frequentist}
...
In the frequentist view, probability is objective and is only discussed for well defined random-experiments \cite{hajek2002interpretations}. In particular, probability is defined as the relative frequency of an event with which it occurs in many trials. Since relative frequencies can vary in different experiments, true probabilities only exist in the limit of infinite trials where the difference in the relative frequencies diminish. In practice, frequentists follow a deductive approach: They introduce a model, i.e. a point in parameter space $\Theta$, and ask for its consistency with observed data, which is considered as the random variable. As such, the focus is set on the \textit{likelihood} $p(\mathcal{D} | \Theta)$ of observing the data given the model parameters. Consequently, frequentists tune model parameters such that the likelihood is maximized. 
...
\label{ML:bias_variance_tradeoff}

\begin{figure}
  \centering
      \includegraphics[width=1.0\textwidth]{./Figures/ml/bias_variance2.pdf}
...
\end{equation}
%
is called the \textit{in-distribution error}. After training, the performance of the model is evaluated computing the cost function with respect to the test set,
%
\begin{equation}
  E_{\text{out}} = \sum_{(\mathbf{x}, \mathbf{y}) \in \mathcal{D}_{\text{test}}}\mathcal{C}(\mathbf{y}, f_{\Theta}(\mathbf{x})) ,
\end{equation}
%
which is called the \textit{out-of-distribution error}. This procedure is known as \textit{cross-validation} and its purpose is to find an unbiased estimate for the predictive performance of the model. In most cases, the out-of-distribution error is greater than the in-distribution error \cite{mehta2019high}.

The general relationship between the training error $E_{in}$ and the generalization error $E_{out}$ is summarized in Fig.~\ref{FIG:ML_bias_variance}~a), where both errors are plotted as a function of the training set size. The following consideration is based on the assumption that the underlying data distribution is sufficiently complex, such that the model will not be able to perfectly reproduce it. Therefore, after an initial drop (excluded in the figure), the in-distribution error $E_{in}$ will increase with the amount of training data, as the model is not powerful enough to capture all the regularities of the training set accurately. On the other hand, the out-of-distribution error $E_{out}$ will decrease with the training set size, as the sampling noise decreases and the training set becomes more representative of the true data distribution. Consequently, both errors will converge to the same value in the limit of infinite training set size \cite{mehta2019high}. The error in the limit of infinite data is called \textit{bias} and the fluctuation of $E_{out}$ due to a limited training set size is referred to as \textit{variance} of the model.

The out-of-distribution error $E_{out}$ is a combination of both, the bias and the variance. An exact decomposition for the expectation of $E_{out}$ can be derived for a regression model trained with mean-square-error (MSE) \cite{mehta2019high}: Consider a data set $\mathcal{D} = \{(\mathbf{x}, \mathbf{y})\}$ sampled from a noisy model
%
\begin{equation}
...
\label{bias_var_decomp}
 ...
 &= \sum_{(\mathbf{x}, \mathbf{y}) \in \mathcal{D}_{\text{test}}} \underbrace{\vphantom{\Big(}\sigma_{\epsilon}^2}_{\textit{noise}} 
 + \underbrace{ \Big( f(\mathbf{x}) - \mathrm{E}_{\mathcal{D}} [ f_{\Theta_{\mathcal{D}}^{*}}(\mathbf{x}) ] \Big)^2 }_{\textit{bias}^2}
 + \underbrace{\mathrm{E}_{\mathcal{D}} \Big[ \Big(f_{\Theta_{\mathcal{D}}^{*}}(\mathbf{x}) - \mathrm{E}_{\mathcal{D}} [ f_{\Theta_{\mathcal{D}}^{*}}(\mathbf{x}) ] \Big)^2 \Big] }_{\textit{variance}} .
\end{align}
%
While trained to minimize the in-distribution error, the ultimate goal for a predictor is to achieve a low out-of-distribution error. All three terms in Eq.~\ref{bias_var_decomp} are positive and therefore each of them has to be minimized. The noise term is not affected by the model and therefore irreducible. The other two terms, the bias and the variance, are reducible. Unfortunately, minimizing both of them simultaneously poses challenges, known as the \textit{bias-variance tradeoff} \cite{mehta2019high}. The bias term 
%
\begin{equation}
...
LVMs are motivated by the assumption that real-world data is generally structured, which implies that it can be described through a lower-dimensional latent distribution $\mathcal{Z}$ supported in $\mathbb{R}^d$. This assumption is known as the \textit{manifold hypothesis}. Formally, it states that high-dimensional data $\mathbf{x} \in \mathbb{R}^D$ tends to lie on a low-dimensional manifold $\mathcal{M} \subset \mathbb{R}^D$ embedded in the high-dimensional ambient space $\mathbb{R}^D$ \cite{fefferman2016testing}. Therefore, real-world data is assumed to have an intrinsic lower dimensionality $d < D$ and the observations $\mathbf{x}$ can be explained through some hidden variables $\mathbf{z} \in \mathbb{R}^d$. However, the actual dimension $d$ of the latent space is typically unknown, as well as the mapping between latent $\mathbf{z}$ and ambient variables $\mathbf{x}$. %As such, $\mathbf{z}$ is referred to as a latent variable and $\mathcal{Z}$ is the latent distribution. 
...
\end{equation}
%
where $\sigma$, $A$, $B$, $a$ and $b$ are the parameters of the model. The value of $\sigma$ is the most important parameter, as it defines the characteristic length scale of the lower-dimensional embedding. In practice, $\sigma$ has to lie within the range of distances characteristic of Gaussian fluctuations and the range dominated by the high dimensionality of the system. The sigmoid function transforms distances such that distances far below or far apart from $\sigma$ are mapped close to zero or unity, respectively. As such, distances in the vicinity of $\sigma$ are highlighted. The minimization of Eq.~\ref{ML:SM_costfunction} scales quadratically with the number of data points. To reduce the computational effort for projections of large data sets, Eq.~\ref{ML:SM_costfunction} is used to obtain low-dimensional positions for a small number of landmark frames. Afterwards, the landmarks can be used to project any other data point $\mathbf{x}$ by minimizing
%
\begin{equation}
  \mathcal{C}(\mathbf{z}) = \sum_i^{N_L} \Big( f(|\mathbf{z} - \mathbf{z}_i|) - F(|\mathbf{x} - \mathbf{x}_i|) \Big)^2  ,  
\end{equation}
%
where $\mathbf{z}_i$ is the projection of a landmark $\mathbf{x}_i$ and $N_L$ is the number of landmarks.

\subsubsection{Time-lagged Independent Components Analysis}
...
Different to FF-NNs, \textit{recurrent neural networks} (RNNs) can have a more complex architecture. RNNs enable cyclic connections between layers, such that a layer can receive information from subsequent layers as well in order to create feedback loops \cite{lipton2015critical}. In particular, a RNN can be described as a recursive process where a recurring function is called in each iteration. This is usually necessary for sequential data, like text or video, where the current state has a dependence on past states. In this thesis, a RNN is used to recursively reconstruct molecular structures. 
...
\end{equation}
%
Note, that Eq.~\ref{rnn_unrolled} implies that multiple states of the network have to be stored simultaneously in memory in order to compute the gradients for training (see Sec.~\ref{NN_training}) making the approach computationally demanding.

\subsubsection{Universal Function Approximator}

%DNNs are universal function approximators, is given by the \textit{universal function approximation theorem} that was first proven by Cybenko in 1989 \cite{cybenko1992approximation}. 
DNNs are universal function approximators. In particular, the classical formulation of the \textit{universal function approximation theorem} states that any continuous function $h$ on a compact set $K$ can be approximated by a FF-NN $f$ with just one hidden layer within arbitrary accuracy $|f(x) - h(x)| < \epsilon$, where $\epsilon > 0$ and $x \in K$ \cite{cybenko1992approximation}. Importantly, the \textit{width} of the hidden layer, i.e. the number of neurons, needs to be unbound in this formulation of the theorem. A dual formulation states that the theorem holds true for bounded width but arbitrary \textit{depth}, i.e. number of layers, as well \cite{lu2017expressive}. 
%While both cases guarantee the existence of an appropriately tuned network capable to approximate any continuous function, DNNs are preferable to shallow networks in most cases, as the hierarchical structure reduces the required number of parameters compared to shallow networks. As such, training becomes more robust and avoids common issues, such as overfitting \cite{mhaskar2017and}.

\subsubsection{Generalization Capability}

Despite being universal function approximators, DNNs also exhibit good generalization behavior. This is surprising, since DNNs are typically over-parameterized, i.e. the number of parameters of the network is significantly larger than the number of training examples. As such, the network could memorize the training samples, i.e. overfit the training data, instead of learning the underlying rules that generate the data. As explained in Sec.~\ref{ML:bias_variance_tradeoff}, regularization is typically used to prevent overfitting by limiting the complexity of the model. This can be achieved by explicit methods, such as penalty terms in the cost function or parameter decay. However, studies have shown empirically that explicit regularization is neither a sufficient nor a necessary condition for the generalization capability of DNNs \cite{zhang2021understanding}. Moreover, implicit regularization techniques, such as stochastic optimization procedures or early stopping of the training, can improve the generalization capability of DNNs, but are also not indispensable \cite{neyshabur2014search}. It can be hypothesized that the generalization capability of DNNs is inherently linked to their architecture \cite{mhaskar2017and}. In conclusion, a more comprehensive theory that explains why DNNs generalize well in practice is an open area of research \cite{kawaguchi2017generalization, neyshabur2017exploring, jakubovitz2019generalization}.
...
A \textit{dense layer} or \textit{fully-connected layer} is the simplest and most common layer of an ANN. Each neuron in a dense layer receives signals $\mathbf{x}$ from all the neurons of the preceding layer. The output $\mathbf{y}$ of a dense layer can be expressed as
%
\begin{equation}
...
Convolutional neural networks (CNNs) have dramatically improved the state-of-the-art in computer vision and made it possible to synthesize photorealistic images of complex objects, such as human faces or animals \cite{voulodimos2018deep, zhang2017stackgan, karras2017progressive, brock2018large}. Therefore, CNNs are the main tool in this thesis to synthesize molecular structures. 

\textit{Convolutional layer} incorporate the idea of local connectivity and translational-equivariance. The idea of local connectivity is inspired by the visual cortex: Neurons are only locally connected to neurons in a restricted area of the previous layer, known as the \textit{receptive field} \cite{baer2009neurowissenschaften}. The receptive field of a neuron becomes bigger the higher it is placed in the hierarchy of the network. Therefore, this architecture is perfectly suited to learn hierarchical pattern in the data. %Moreover, the limited receptive field can be used to enforce translation-equivariance of the learned patterns. 

A convolutional layer consists of a bank of parameterized \textit{filters} that are sliced over the input. In the following, the two dimensional case is considered, i.e. images $\mathbf{X}_j \in \mathbb{R}^{N_x \times N_y}$, where $N_x$ is the width and $N_y$ is the height of the image. The subscript $j \in \{0, .., N_c\}$ is the index for the $N_c$ \textit{feature channels}. The different feature channels provide different views on the data, such as the different color channels of a red-green-blue (RGB) image. At each position, the discrete convolution of the filter with the segment of the image it overlaps is computed and stored into a so called \textit{feature map}. Note that the parameters of each filter are shared between all neurons of the layer, which enforces equivariance with respect to translations of the learned patterns. The bank of filters connecting the $j$th feature channel of the input with the $i$th feature channel of the output is denoted with $\mathbf{K}_{i,j} \in \mathbb{R}^{m_x \times m_y}$, where $m_x$ is the width, $m_y$ is the height of the filters. The $i$th feature map $\mathbf{Y}_i \in \mathbb{R}^{n_x \times n_y}$ of the output can be computed as
%
\begin{equation}
...
\begin{itemize}
  ...
  \item Strides: While the filter is sliced over the input, the step size $S$, called \textit{stride}, for the translation can be greater than one effectively reducing the output size, or smaller than one effectively increasing the output size.
\end{itemize}
...
The cost function $\mathcal{C}$ maps the output of the network $f_{\mathbf{\Theta}}(\mathbf{x})$ to a real number representing the error. While more details about the cost function used in this thesis will be revealed in in Sec.~\ref{theory_ML:generative_modeling}, two requirements it has to fulfill shall be noted to in order to explain the training procedure of ANNs: (1)~It has be differentiable in order to apply gradient-based optimization algorithms. (2)~It has be written as an average
%
\begin{equation}
...
A naive, direct computation of the gradients with respect to each weight individually is computationally expensive and not feasible for DNNs. To circumvent these limitations, \textit{backpropagation} (BP) was invented to efficiently compute the gradients for DNNs \cite{werbos1982applications}. It is based on the chain rule and benefits from the nested structure of a NN, which enables to compute the gradients layer by layer: Starting from the last layer, it iterates backward through the network, whereby avoiding duplicate and unnecessary intermediate calculations.
...
A widely used stochastic optimization algorithm is \textit{stochastic gradient descent} (SGD): The size of a training set is typically large and therefore, the gradient in Eq.~\ref{gradient_descent} can not be computed over the whole set in each optimization step. Therefore, the training set is usually shuffled and split into mini-batches $T = \{(\mathbf{x}_1, \mathbf{y}_1), \dots, (\mathbf{x}_n, \mathbf{y}_n)\}$ of size $n$, where $n$ is treated as a hyper parameter. Since the gradient is not computed exactly but only for a part of the data, the optimization algorithm contains a stochastic element. However, the gradient estimation can vary significantly for different training batches in SGD. To this end, SGD can be augmented by a momentum term 
%
\begin{equation}
...
\label{theory_ML:generative_modeling}

While DNNs will built the core of the reverse-mapping scheme developed in this thesis, an adequate cost function to train the model is required. In particular, the model has to be trained such that it can synthesize further samples from the fine-grained distribution of a molecular system. To this end, concepts of \textit{generative modeling} are applied to learn this distribution from training data. 
...
Since only generative models are capable of generating new instances of the underlying distribution, the remainder of this chapter focuses on the generative approach. More specifically, the following discussion is restricted to \textit{Deep Generative Models} (DGMs), i.e. generative models based on DNNs.

%However, it is straightforward to compute marginalized probabilities 
%
%\begin{equation}
...
A DGM provides an estimate for the probability $p_{\Theta}(\mathbf{x})$ of an observation $\mathbf{x}$. The general goal is to approximate the true probability distribution, i.e. to find the optimal parameters $\Theta^{*}$ such that  $p_{\Theta^{*}}(\mathbf{x}) \approx p_{\mathcal{X}}(\mathbf{x})$.
A major route to train a DGM is the frequentist approach of maximizing the data likelihood. The underlying idea of this approach is to find a model that best explains the observed data. In particular, the likelihood $\mathcal{L}$ for the data under the parametric model can be written as
%
\begin{equation}
...
%
\begin{equation}
 \Theta^{*} = \underset{\Theta}{\text{argmax}} \; \mathcal{L} = \underset{\Theta}{\text{argmin}} \; - \sum_i^N log ( p_{\Theta}(\mathbf{x}) ) .
\end{equation}
%
...
\end{align}
%
and the scaling factor $\frac{1}{N}$ is irrelevant, as it does not affect the $\underset{\Theta}{\text{argmin}}$ operation. Both, cross-entropy and relative entropy, reach a minima when the two distributions match, i.e. $p_{\Theta}(\mathbf{x}) = p_{\mathcal{X}}(\mathbf{x})$, making the negative logarithm of the likelihood a well suited cost function for generative models. However, how can the likelihood function be assessed?

%. As such, maximizing the likelihood is well suited to learn the data distribution $\mathcal{X}$.  
%In the maximum likelihood approach, the parameters $\Theta$ are optimized to maximize $\mathcal{L}$:
%
%\begin{equation}
...
%The most prominent member of implicit generative models is the generative adversarial approach.
...
\begin{align}
  log(p_{\Theta}(\mathbf{x})) &\geq \mathbb{E}_{e_{\Psi}(\mathbf{z}|\mathbf{x})} \Big[ log \big( \frac{p_{\Theta}(\mathbf{x},\mathbf{z})}{e_{\Psi}(\mathbf{z}|\mathbf{x})} \big) \Big] \\
    &= \underbrace{ \mathbb{E}_{e_{\Psi}(\mathbf{z}|\mathbf{x})} \Big[ log \big( p_{\Theta}(\mathbf{x}|\mathbf{z})\big) \Big] } _\text{reconstruction} -  \underbrace{ \phantom{\Big[}  \text{KL}(e_{\Psi}(\mathbf{z}|\mathbf{x})||p_{\mathcal{Z}}(\mathbf{z}))  }_\text{regularization}
\end{align}
...
The main drawback of VAEs is a potentially large gap between the ELBO used for optimization and the actual likelihood resulting in a model that differs significantly from the true distribution \cite{ruthotto2021introduction}. Empirically, vanilla VAEs have a tendency to ignore some of the latent variables and/or produce blurred samples in ambient space \cite{asperti2021survey}.

%Note that both terms might be in a conflict with each other. 
\subsubsection{Markov Chain approximation}
  
Some generative models generate samples using a Markov chain technique: A sample $\mathbf{x}$ is repeatedly updated according to some transition operator $\mathbf{x}' \sim q(\mathbf{x}'|\mathbf{x})$. An example is the Boltzmann machine (BM), which is a LVM that consists of binary units $u_i$ connected with each other \cite{fahlman1983massively, ackley1985learning, hinton1986learning, hinton1984boltzmann}. The weights $\Theta$ and thresholds $\Phi$ of the network are essentially the parameters of an energy function $E(\Theta, \Phi)$. In particular, $E(\Theta, \Phi)$ describes a spin-glass model with an external field 
%
\begin{equation}
 E(\Theta, \Phi) = - \Big( \sum_{i<j} \Theta_{i,j}u_i u_j + \sum_i \Phi_i u_i \Big)  .
\end{equation}
%
...
%The units of a BM are divided into visible units receiving information from the environment and hidden (latent) units.
  
\subsection{The Generative Adversarial Network: An Implicit Generative Model}
\label{ML_GAN}

\textit{Generative adversarial networks} (GANs) were introduced by Ian Goodfellow \emph{et al.} in 2014 \cite{goodfellow2014generative}. They have become one of the most successful implicit generative models known in the ML community \cite{zhang2017stackgan, karras2017progressive, creswell2018generative, gui2021review}. Their ability to generate photorealistic images of complex objects have motivated their usage in this thesis as the main training procedure to generate high fidelity molecular structures.
...
\label{gan_loss_binary}
  \mathcal{C} \big( g_{\Theta}, c_{\Psi} \big) = \mathbb{E}_{\mathbf{x} \sim \mathcal{X}} \Big[ log(c_{\Psi} \big( \mathbf{x}) \big) \Big] + \mathbb{E}_{\mathbf{z} \sim \mathcal{Z}} \Big[ log\Big(1 - c_{\Psi} \big( g_{\Theta}(\mathbf{z}) \big) \Big) \Big] .
\end{equation}
%
As a result, the GAN training becomes a mini-max game, where the discriminator aims at maximizing $\mathcal{C} \big( g_{\Theta}, c_{\Psi} \big)$, while the generator tries to minimize $\mathcal{C} \big( g_{\Theta}, c_{\Psi} \big)$ \cite{goodfellow2014generative}:
%
\begin{equation}
  \Psi^{*} = \underset{\Psi}{\text{arg max}} \; \mathcal{C} \big(g_{\Theta}, c_{\Psi} \big) \;\; \text{and} \;\; \Theta^{*} = \underset{\Theta}{\text{arg min}} \; \mathcal{C} \big( g_{\Theta}, c_{\Psi} \big)
\end{equation}
%
...
\begin{equation}
  ...
  JS \big( p_{\mathcal{X}}|| p_{\Theta} \big) = \frac{1}{2} D\Big(p_{\mathcal{X}} \Big| \Big| \frac{p_{\mathcal{X}} + p_{\Theta}}{2} \Big) + \frac{1}{2} D\Big(p_{\Theta} \Big| \Big| \frac{p_{\mathcal{X}} + p_{\Theta}}{2} \Big) .
\end{equation}
%
...
\label{gan_nonsaturating_loss}
    \mathcal{C}\big(g_{\Theta}\big) = \mathbb{E}_{\mathbf{z} \sim \mathcal{Z}} \Big[- log \Big( c_{\Psi} \big( g_{\Theta} ( \mathbf{z}) \big) \Big) \Big] 
\end{equation}
%
...
\begin{equation}
\label{EMD}
  W\big(p_{\mathcal{X}}, p_{\Theta} \big) = \underset{\gamma \in \Gamma (p_{\mathcal{X}}, p_{\Theta})}{\text{inf}} \;  \mathbb{E}_{(\mathbf{x}, \mathbf{x'}) \sim \gamma} \big[ \lVert \mathbf{x} - \mathbf{x'} \rVert \big] ,
\end{equation}
%
...
\label{wasserstein_duality}
  W\big(p_{\mathcal{X}}, p_{\Theta}\big) = \underset{f \in \text{Lip}(f)\leq 1}{\text{max}} \mathbb{E}_{\mathbf{z} \sim \mathcal{Z}} \Big[f\big(g_{\Theta}(\mathbf{z}) \big)\Big] - \mathbb{E}_{\mathbf{x} \sim \mathcal{X}} \Big[f(\mathbf{x}) \Big]
\end{equation}
%
...
\label{wasserstein_loss}
  \underset{\Theta}{\text{min}} \; \underset{\Psi}{\text{max}} \; \mathbb{E}_{\mathbf{z} \sim \mathcal{Z}} \big[c_{\Psi}\big(g_{\Theta}(\mathbf{z})\big) \big] - \mathbb{E}_{\mathbf{x} \sim \mathcal{X}} \big[c_{\Psi}(\mathbf{x})\big] .
\end{equation}
%
Various different methods exist to ensure the 1-Lipschitz continuity. A popular approach is \textit{gradient penalty} (GP) that introduces an additional term to the cost function of the critic \cite{gulrajani2017improved}. A differentiable function is one-Lipschitz if and only if it has gradients everywhere with norm at most one. A soft version of this constraint can be enforced by a penalty on the gradient norm
%
\begin{equation}
\label{gradient_penalty}
  \mathcal{C}_{\text{gp}}(c_{\Psi}) = \mathbb{E}_{\bar{\mathbf{x}} \sim \bar{\mathcal{X}}} \big[ \big(|| \nabla_{\bar{\mathbf{x}}} c_{\Psi}(\bar{\mathbf{x}})||_2 - 1\big)^2  \big] ,
\end{equation}
%
