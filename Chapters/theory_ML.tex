...
where $p(\mathcal{D}) = \int p(\mathcal{D} | \Theta) p(\Theta) d \Theta$ is the \textit{evidence} or marginal likelihood \cite{gelman1995bayesian}. In practice, bayesians perform a maximum a posteriori (MAP) estimation, which bypasses the cumbersome computation of the posterior distribution, but tries to find the point estimates of the parameters  that maximize the posterior distribution instead. Note that MAP estimation can be treated as the maximum likelihood approach, when a uniform prior distribution $p(\Theta)$ is assumed.
