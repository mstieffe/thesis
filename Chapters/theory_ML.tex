...
that maps the output $\mathbf{y}$ of $f_{\Theta}(\mathbf{x})$ to a real number representing the error the model has made on $\mathbf{x}$. That is, $\mathcal{C}$ is used to judge the performance of the model. During training the parameters of the model $\Theta$ are tuned such that the cost function is minimized, ultimately aiming at discovering the optimal set of parameters $\Theta^{*}$.
...
Switching between representations of different resolutions is a hallmark of MS modeling. However, a similar concept is also well known in the ML community, where hidden and typically lower dimensional representations are described by so called \textit{latent variables}. ML models that relate latent variables with observable varibables are refered to as \textit{latent variable models} (LVM).\cite{van2009dimensionality, ge2018process} 

LVMs are motivated by the assumption that real-world data is generally structured, which implies that it can be described through a lower-dimensional distribution $\mathcal{Z}$ supported in $\mathbb{R}^d$. This assumption is known as the \textit{manifold hypothesis}. Formally, it states that high-dimensional data $x \in \mathbb{R}^D$ tends to lie on a low-dimensional manifold $\mathcal{M} \subset \mathbb{R}^D$ embedded in the high-dimensional space $\mathbb{R}^D$.\cite{fefferman2016testing} Therefore, real-world data is assumed to have an intrinsic lower dimensionality $d < D$ and the oberservations $x$ can be explained through some hidden variables $z \in \mathbb{R}^d$. 

The point $z$ that maps to a point $x$ is unknown in general. As such, $z$ is refered to as a latent variable and $\mathcal{Z}$ is the latent distribution. The high-dimensional space $\mathbb{R}^D$ that supports the actual distribution $\mathcal{X}$ is called \textit{ambient space}. The low-dimensional space $\mathbb{R}^d$ that supports the latent distribution $\mathcal{Z}$ is refered to as the \textit{latent space}. The actual dimension $d$ of the latent space is typically unknown. 

The concept of coarse-graining and latent variables ultimately follow the same philosophy: Both aim at reducing the complexity by mapping a high dimensional representation to a lower dimensional, simpler representation that still captures the essential features while noise and redundant features are removed. However, coarse-grained representations of molecular systems are frequently based on physical and chemical intuition, whereas dimensionality reduction based on LVMs seeks to discover hidden and potentially lower-dimensional variables by minimizing a cost function instead. Recently, LVMs have been applied to the coarse-graining of molecular systems.\cite{wang2019coarse}

Similarly to the concept of backmapping, LVMs can be applied to the inverse task as well, i.e. to generate a new instance of $\mathbf{x}' \sim \mathcal{X}$. For this purpose, the generative process is decomposed into two steps: (1) Draw a sample $z \sim \mathcal{Z}$ from the latent distribution with probability $p_{\mathcal{Z}}(z)$. (2) Introduce a parametrized mapping function 
...
In many cases a tractable distribution, such as a Gaussian distribution, is assumed to resemble the latent distribution. However, the proposed $\mathcal{Z}$ can differ signficantly from the actual manifold the data resides. Therefore, a linear transformation is not sufficient to deform $\mathcal{Z}$ such that is resembles $\mathcal{X}$.\cite{lopez2020deep} Consequently, a highly nonlinear transformation is seeked. In addition, deriving $g_{\Theta}$ from first principles is infeasible for most real-world applications. 

The strong similarity between the concept of latent and coarse-grained variables suggests to apply LVMs to MS modeling. In this thesis, the general approach is to build a LVM for the inverse task, i.e. for the backmapping of molecular structures. It will be shown in Sec. \ref{} that Deep Neural Networks provide an excellent tool learn such complex transformations, as they have the ability to efficiently approximate functions in high dimensions.\cite{hornik1989multilayer, cybenko1992approximation, naitzat2020topology} 
...
Reducing the dimensionality has become an important aspect in modern data analysis. Algorithms designed for this purpose are numberous and range from classical linear techniques, such as principle component analysis, to non-linear ML approaches, such as LVMs. While this section being far from complete, it shall give a brief overview of some techniques that are used in this thesis for analysiing molecular configurations.

\subsubsection{Principle Component Analysis}

One of the most commonly used methods for dimensionalty reduction is principle component analysis (PCA). It performs a change of basis by projecting the data onto a linear combinations of the original basis vectors called principle components. The new coordinate system is thereby chosen such that they align with the directions showing the highest variance in the data set. PCA is typically used for dimensinality reduction by discarding principle components with low variance, i.e. the new representation preserves most of the dataś variance.

\subsubsection{Sketch-Map}

Despite its wide range of applications, linear dimensionality reduction techniques like PCA are typically insufficient to capture the global structure of data obtained from MD trajectories, since the accessable regions of phase space can have a complex, nonlinear structure with nonuniform dimensionality.\cite{garcia1992large, hegger2007complex, rohrdanz2011determination} As such, nonlinear dimensionality reduction techniques are more appropriate to analyze the phase space of molecular systems. 

A promising approach introduced by Ceriotti \emph{et al.} is called \textit{Sketch-Map}.\cite{ceriotti2011simplifying, tribello2012using} Related to the curse of dimensionality, Ceriotti et al state that two points in phase space with small distance correspond to thermal fluctuations, while large distances are uniformily distributed. Consequently, the focus has to be set on an intermediate scale capturing most of the valuable topological information of the phase space. Such a characteristic length scale is found by minimizing the cost-function
...
Various different methods exists to ensure the 1-Lipschitz continuity. A popular approach is \textit{gradient penalty}, where an additional term is introduced to the cost function of the critic. A differentiable function is one-Lipschitz if and only if it has gradients everywhere with norm at most one. A soft version of this constraint is enforced with a penalty on the gradient norm
...
where $\bar{\mathbf{x}}$ is interpolated linearly between pairs of points $\mathbf{x}$ and $g_{\Theta}(\mathbf{z})$. The additional term in the loss function may be considered a regularizer for the complexity of the critic $c_{\Psi}$.\cite{}
...
\label{theory_ML_GAN_limitations}
