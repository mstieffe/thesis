...
The definition for generative models is often opposed to discriminative models. While the former aims to learn about the dependencies of all the variables in a system, the goal of the latter is to introduce decision boundaries for different classes. Generative models are typically linked to unsupervised training and discriminative models to supervised trainig, but a clear dichotomy is not always possible.

Formally, consider a set of labelled data $\{(x,y)\}$ following the joint probability distribution $p(x,y)$, where $x$ is an observation and $y$ the corresponding class label. The goal of a discriminative model is to learn the conditional probability $p(y|x)$ of the class labels $y$ given the obsersvation $x$. This can be achieved either explicitly or implicitly by directly mapping the oberservations $x$ to a class label $y$. To fullfil this goal, a discriminative model does not necessarily have to learn about the depencies of all the variables in the system as it only has to make use of the variables that are important for labeling the observations.

Generative approaches on the other hand aim to model the joint probability $p(x,y)$ (or simply $p(x)$ if no labels are given). They are framed generative as the probability $p(x,y)$ can be used to generate new instances of the underlying distribution. However, it is straighforward to compute marginalized probabilities $p(x) = \sum_y p(x,y)$ or conditional probabilities $p(y|x) = \frac{p(x,y)}{p(x)}$. Therefore, generative models can be used for classification as well. 
