...
ML is an umbrella term for many different algorithms, such as Kernel methods, decision trees or Artificial Neural Networks (ANNs). In this work we will focus on the latter. 


\section{Artificial Neural Networks}

Nervous systems have the fascinating ability to organize themself and learn from experience. This has motivated researchers over the past century to develop artificial models of its biological counterpart.
...
Nervous systems are built up from a large number of interconnected cells, called neurons. The human brain, as an example, consists of $\sim 10^{11}$ neurons. A single neuron is a very simple processing unit, but a network of neurons becomes a powerful information processing system: Signals from the environment captured by sensory cells are encoded and processed by the network to create an appropriate response. 

An illustration of a biological and an artificial neuron is shown in Fig.\ref{fig_neuron}. Both neurons can receive, process and transmit signals. The main parts of a biological neuron are the dendrites, the cell body and the axon. Dendrites are thin fibers connected via synapses with the axons of thousands of other neurons. Signals captured at the dendrites are weighted by the synapses and can either increase or decrease the electrical cell potential. If a specific threshold potential is reached the axon fires a signal. Similarly, an artificial neuron works:
