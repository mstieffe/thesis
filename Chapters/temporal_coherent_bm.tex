...
A variational autoencoder (VAE) architecture (Sec. \ref{}) is used instead of the generative adversarial approach. To this end, an encoder network $e$ is introduced to generate latent samples $\mathbf{\hat{z}}_{t}$ based on the current target frame $\mathbf{r}_{t}$. More specifically, the input for the encoder consists of $(\mathbf{r}_{t}, \mathbf{R}_{t}, \mathbf{r}_{t-\tau})$. This allows the model to be trained end-to-end based on a reconstruction loss for the current state $\mathbf{r}_{t}$ and does not rely on an additional critic network.

While the encoder $e_{\Psi}$ is indispensable during training in order to implement the reconstruction loss, it is omitted at inference time and the latent sample $\mathbf{z}$ is drawn from a prior distribution. Specifically, $\mathbf{z}$ is drawn from a Gaussian Mixture Model fitted to the latent distribution implied by the encoder, instead of the assumed prior $p(\mathbf{z}) \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$. This process ensures that the decoder operates within densely sampled latent space regions. The actual reverse-mapping of the coarse-grained trajectory $(\mathbf{R}_0, \mathbf{R}_{\tau}, \mathbf{R}_{2\tau}, \dots )$ is performed solely by the decoder. 

\subsubsection{Cost-function}

 In particular, the deployed cost-function $\mathcal{C}$ is constructed as
...
where $\tau$ is the lagtime between frames. Fig. \ref{FIG:TCBM_velocities} displays the intra-frame velocity distributions obtained for the reference trajectory and both reconstructed test sets. In addition, intra-frame velocity distributions obtained for a baseline backmapping method are shown. The baseline method is fragment-based and treats each frame separately. In particular, a library consisting of 40000 pairs of equilibrated atomistic and associated coarse-grained frames is generated for both systems. Backmapping of a coarse-grained frame is performed by selecting the closest matching coarse-grained structure from the library in terms of root-mean-square deviation and projecting the corresponding atomistic structure onto the coarse-grained representation. 
...
The results obtained for CLN are displayed in panel (b). Temporal coherent backmapping yields a velocity distribution for the in-distribution test set that matches remarkably well with the reference distribution. Similarly to ADP, the baseline method is not able to reproduce the reference velocities. Velocities obtained for the generalization set deploying the ML model are significantly higher compared to the reference system.
