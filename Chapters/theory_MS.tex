...
The Kremer-Grest (KG) model is top-down model widely used to study generic polymer properties. The model represents polymers as chains of beads connected via non-linear springs. The spring potential is tuned such that crossing of two polymer chains is avoided in order to correctly simulate the dynamics long-chain polymer melts, in especially entanglement effects.
...
where $\Theta$ is the the bond angle and $\kappa$ defines the stiffness.

While the KG model is a generic model to study universal phenomenas of polymers, the stiffness can be used to relate the model to specific polymer species. 
