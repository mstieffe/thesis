...
Reducing the resolution is just one side of the MS philosophie that allows to adjust the focus to larger scales. To close the loop, a reverse-mapping is required that allows to go the other direction as well. Such a reverse-mapping, also refered to as \textit{Backmapping}, can be regarded as a magnifying glass to zoom in to the molecular system. To this end, the backmapping scheme has to reintroduce the missing details that have not been captured in the coarse-grained model.
