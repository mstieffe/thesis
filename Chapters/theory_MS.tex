...
While mapping from a higher to a lower resolution is typically straightforward, the opposite direction is more challenging. Formally, let $\{A_{I} = (\mathbf{R}_I, C_I) | I = 1, \dots, N \}$ denote the set of $N$ coarse-grained beads. Each bead has position $\mathbf{R}_I$ and an associated list of features $A_I$. The features $A_I$ can include various attributes, such as the chemical element, the connectivity or associated force field parameters. Similarly, atomistic structures are denoted as the set of $n$ atoms $\{a_{I} = (\mathbf{r}_i, c_i) | i = 1, \dots, n \}$, with position $\mathbf{r}_i$ and attributes $c_i$. A backmapping function $\phi$ takes the coarse-grained information $(\mathbf{A}_1, \dots, \mathbf{A}_N)$ as well as the desired atomistic attributes $(\mathbf{c}_1, \dots, \mathbf{c}_n)$ as input and generates a set of coordinates $(\mathbf{r}_1, \dots, \mathbf{r}_n)$,

\begin{equation}
  \phi \Big( (\mathbf{A}_1, \dots, \mathbf{A}_N), (\mathbf{c}_1, \dots, \mathbf{c}_n) \Big) = (\mathbf{r}_1, \dots, \mathbf{r}_n)
\end{equation}

Deriving the function $\phi$ is not a trivial task, as it is constrained by two important aspects. First of all, the mapping has to be consistent, i.e. the missing degrees of freedom have to be reinserted along the coarse-grained degrees of freedom. In other words, applying the coarse-grained mapping on the backmapped structure has to yield the original coarse-grained structure,
