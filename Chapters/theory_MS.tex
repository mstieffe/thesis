...
\textit{Molecular Dynamics} (MD) simulation circumvents these issues by numerically intergrating Newtons equations of motion allowing to analyze the physical movement of atoms or molecules. To this end, it is common to introduce forcefields that determine the interactions between the particles, initialize the system and use algorithms to evolve the system in time. After the system is equilibrated, i.e. it has evolved for a sufficient amount of time such that macroscopic proties do not change anymore, the actual measurement of the observable quantity of interest can be performed. However, the algorithm and parameters for the numerical intergration have to be taken with great care in order to minimize the effects of cumulative errors.

\cite{shell2019lecturenotes}, \cite{frenkel2001understanding}

\subsection{Molecular Force Fields}

Calculating forced acting on the particles is crucial to evolve the system in time. To this end, a definition for the potential energy function is required describing the interactions between the particles. These potentials can be defined on vary different levels of resolution. In the following, the classical description is used, which ignores the motion of the electrons and focuses solely on the motion of the nuclei. In this regard, the classical description approximates the effect of electrons as a potential energy surface representing the quantum ground-state. This description is reasonable in cases where the Born-Oppenheimer approximation is valid and the electronic structure is not of interest. Furthermore, bond breaking or forming is prohebitted. 

Given the potential energy function $U(\mathbf{r})$ the forces $\frac{d \mathbf{p}}{d \mathbf{t}}$ are calculated taking the derivative with respect to the particle positions $\mathbf{r}$

\begin{equation}
  \frac{d \mathbf{p}}{d \mathbf{t}} = \frac{d U (\mathbf{r})}{d \mathbf{r}} .
\end{equation}
